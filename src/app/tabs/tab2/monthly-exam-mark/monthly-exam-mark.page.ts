...
@Component({
  selector: 'app-monthly-exam-mark',
  templateUrl: './monthly-exam-mark.page.html',
  styleUrls: ['./monthly-exam-mark.page.scss'],
})
export class MonthlyExamMarkPage implements OnInit {
...
if(this.marksForm.valid){
  this.loader = true;
     ...
     this._api.monthAddMark(this.marksForm.value).subscribe(v=>{
       alert(v.message)
       this.loader = false;
       this._router.navigate(['/tabs/tab2/monthly-exam'])
     },er=>{
       this.loader = false;
     })
  }
  ...
  studentDetails(data: any): FormGroup {
    return this._fb.group({
      ...
      date: [this.filterForm.value.date], // Default can also be ''
